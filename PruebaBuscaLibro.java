...
public class PruebaBuscaLibro {
	...
	public static void main(String[] args) {
		...
		DescripcionLibro descripcionLibroBuscado = 
			new DescripcionLibro("un mundo feliz", 
								 "aldous huxley", null, null, -1);
									   
		List libros = inventario.buscar(descripcionLibroBuscado);
		...
		if(!libros.isEmpty()) {
			...
			for(Iterator i = libros.iterator(); i.hasNext();) {
				...
				System.out.println("Se ha encontrado el libro de " +
									descripcion.getTitulo() + "\n" +
									"Escrito por " + descripcion.getAutor() + "\n" +
									"de la editorial " + descripcion.getEditorial() +
									"\nen formato " + descripcion.getFormato() +
									"\ncon " + libro.getPaginas() + " páginas" +
									"\n\n...\n");
			}
			
		} else {
			System.out.println("Lo sentimos, no se han encontrado resultados" +
								" para el libro que busca.");
		}
		
		
	}
	
	private static void inicializaInventario(Inventario inventario) {
		
		inventario.addLibro("84-7888-744-X", 896,
		...
		inventario.addLibro("978-607-14-1116-7", 200,
		...
		inventario.addLibro("978-607-7835-61-5", 464,
		...
		inventario.addLibro("970-627-200-1", 256,
		...
		inventario.addLibro("970-666-777-6", 192,
